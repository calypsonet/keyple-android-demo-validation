...
class MockSamReaderRepositoryImpl @Inject constructor(private val readerObservationExceptionHandler: CardReaderObservationExceptionHandlerSpi) :
    ...
    override suspend fun initSamReaders(): List<Reader> {
        val samPlugin =
            SmartCardServiceProvider.getService().getPlugin(MockSamPlugin.PLUGIN_NAME)
    ...
    override fun getSamReader(): Reader? {
        return if (samReaders.isNotEmpty()) {
            val filteredByName = samReaders.filter {
                it.name == MockSamReader.READER_NAME
            }
    ...
    internal class ReaderConfigurator : ReaderConfiguratorSpi {
        ...
        override fun setupReader(reader: Reader) {
            ...
            try {
                reader.getExtension(MockSamReader::class.java)
            } catch (e: Exception) {
                Timber.e(
    ...
    companion object {
        const val SAM_READER_NAME_REGEX = ".*SamReader"
    }
}
